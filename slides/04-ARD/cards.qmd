...
- Displays for hierarchical data report on **each level** of the hierarchy (Any AE Overall, by System Organ Class, by Preferred Term)

- Producing that by hand means several calls to the simpler `ard_hierarchical()` / `ard_hierarchical_count()` functions, each on a **different subset** of the data (e.g. Overall rates need 1 record per subject in ADAE)
