...
## Why plan first? {.smaller}

One request, **four packages and three skills** deep. The agent makes a dozen
...
On script 3 you notice it **derived its own ADSL** instead of starting from
`pharmaverseadam::adsl` — so scripts 1 and 2 need reworking too.
...
You fix the ADSL source in one line, then hit approve.
...
## One prompt, in Plan mode {.smaller}
...
Nobody named a skill. The agent matched them from the `description:` front
matter of what is checked into `.agents/skills/`:
...
>
> 1. Step 3 — don't derive ADSL from scratch. Start from
>    `pharmaverseadam::adsl` and merge the DM variables in.
> 2. Step 4 — summarise by `TRT01P`, not `ARM`, and keep the
