...
-   By the end of this ADaM section you will have:
    - **Gained** an understanding of how to use `admiral`, `metatools/metacore`, and `xportr` packages - together with an AI assistant as a pair-programmer - to create ADaMs
    ...
    - **Practiced** prompting an AI assistant to explain, adapt, and troubleshoot `admiral` functions instead of hand-writing every line.
    - **Checked** in on our fav participant - Barb.
    - **Completed** two short exercises using functions from admiral package, leaning on an AI assistant when you get stuck.
:::
...
## How We'll Use It Today {.smaller}

::: incremental
-   Pulling up `admiral`/`metatools`/`xportr` function docs and arguments on demand, instead of memorizing them.
-   Explaining *why* a derivation works the way it does with our `ADSL`/`ADVS` data.
-   Helping during the two check-in exercises when you get stuck on `derive_vars_dtm()` or `derive_param_computed()`.
-   Two live demos below - asking it to adapt a script snippet for a new date variable, then a new computed parameter.
:::

## Live Demo 1 - Adapting `derive_vars_dtm()` (in `ADSL`) {.smaller}

* Our script imputes `EXSTDTC`/`EXENDTC` with `time_imputation = "last"` (Lines: 57-69).
* We'll ask Posit Assistant, live, in the console:

::: {.codewindow style="font-size: 0.75em;"}
```
Our adsl.R script calls derive_vars_dtm() on EXSTDTC and EXENDTC with
time_imputation = "last". Write a similar derive_vars_dtm() call for
DSSTDTC that imputes a missing day to the first of the month and a
missing time to noon, and explain how flag_imputation would differ
from what we used above.
```
:::

* Goal: watch it read our actual script/objects, propose code, and explain the imputation flags - rather than us reciting the docs.

## Live Demo 2 - A New Computed Parameter (in `ADVS`) {.smaller}

* Our script derives `BMI` with `derive_param_computed()` from `WEIGHT`/`HEIGHT` (Lines: 100-111).
* Before we reveal the MAP check-in exercise solution, we'll ask Posit Assistant:

::: {.codewindow style="font-size: 0.75em;"}
```
Using the same pattern as our BMI derive_param_computed() call in
advs.R, write a new call that derives Pulse Pressure
(PP = SYSBP - DIABP) as a new parameter, PARAMCD = "PP", from the
SYSBP and DIABP records.
```
:::

* Goal: a second, independent example of adapting a derivation - deliberately different from the MAP formula you'll solve by hand shortly.
...
::: incremental
-   I will **pull out** certain functions of the  [`ADSL`](https://github.com/posit-conf-2026/pharmaverse/blob/main/slides/03-ADaM/scripts/adsl.R) and [`ADVS`](https://github.com/posit-conf-2026/pharmaverse/blob/main/slides/03-ADaM/scripts/advs.R) R scripts.
    - Let's take a quick peek at the [scripts](https://github.com/posit-conf-2026/pharmaverse/tree/main/slides/03-ADaM/scripts).
    -   I will discuss **certain** functions and function arguments from [scripts](https://github.com/posit-conf-2026/pharmaverse/tree/main/slides/03-ADaM/scripts) for deriving variables/records - and show how an AI assistant can pull up the same explanations on demand. 
    -   Rather than memorizing every argument, we'll **ask an AI assistant** to summarize other powers. 💡
...
-   Brief notes on the code in script and exercises:
    ...
    -   Exercises will need some minor updates - try asking an AI assistant to help fill in the blanks before hand-coding a fix.
...
- Let's take a quick peek at [metadata](https://github.com/posit-conf-2026/pharmaverse/tree/main/slides/03-ADaM/metadata)
...
- These are also well-documented, widely-used packages - meaning AI assistants tend to know them well and can help you write idiomatic `admiral` code faster.
:::
