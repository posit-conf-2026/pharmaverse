...
[11:45 - 11:52]{.pv-agenda-time}
:::
::: {.pv-agenda-item}
[📊]{.pv-agenda-ico}
[`ADVS`]{.pv-agenda-title}
[11:52 - 12:23]{.pv-agenda-time}
:::
::: {.pv-agenda-item}
[✏️]{.pv-agenda-ico}
[New Records Exercise]{.pv-agenda-title}
[12:23 - 12:30]{.pv-agenda-time}
:::
::: {.pv-agenda-item}
[🍽️]{.pv-agenda-ico}
...
* Breaking down `filter_add`:
  - `EXDOSE > 0` - keep any record with a real (non-zero) dose
  - `EXDOSE == 0 & str_detect(EXTRT, "PLACEBO")` - **or** keep a zero-dose record only when it's explicitly a placebo administration, so a placebo subject still gets a valid "treatment start" record, but a skipped/withheld dose doesn't
  - `!is.na(EXSTDTM)` - **and** only consider records that actually have a start datetime to order/select on
  - Together, this narrows the candidate pool *before* `mode = "first"` picks the earliest qualifying record per subject
