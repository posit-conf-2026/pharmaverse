...
::: {.incremental style="list-style: none; padding-left: 0;"}
-   🎯 **Gained** an understanding of how to use `admiral`, `metatools/metacore`, and `xportr` packages - together with an AI assistant as a pair-programmer - to create "submission compliant" ADaMs
...
-   ✅ **Completed** exercises using functions from admiral package, leaning on an AI assistant when you get stuck.
:::
...
-   🕥11:15 - 11:35 Subject-level dataset: `ADSL`

-   🕚11:35 - 11:50 Hands-on Exercise

-   🕦11:50 - 12:10 Basic Data Structure: `ADVS`

-   🕚12:10 - 12:30 Hands-on Exercise

-   🕚12:30 Lunch!
:::

## Meet Your Co-Pilot: Posit Assistant

<div style="text-align: center;"> <img src="images/posit-assistant-logo.png" alt="Posit Assistant" style="width: 400px;"/> </div>

<div style="text-align: center; font-style: italic;"> It won't judge your variable names. Much. </div>

::: incremental
-   `Posit Assistant` is the AI assistant built into RStudio (and Positron) that we'll lean on throughout this section.
-   It connects directly to your **live R session** - it can see your loaded packages, objects, and data, not just your code.
-   It combines conversational AI with **code execution**, so it can run, inspect, and iterate on `admiral` code alongside you rather than just describing it.
-   Useful for: explaining unfamiliar functions/arguments, adapting example code to new variables, and troubleshooting errors as they come up.
:::
...
class C highlight;
classDef highlight fill:#ff4d4d, stroke:#333, stroke-width:2px;
```
...
::: incremental
-   Data: SDTMs &rarr; `ADSL` &rarr; `ADVS`
-   Specs drive every derivation - we'll peek at ours shortly
-   pharmaverse packages that play nice with tidyverse and base R
-   Two exercises today: `derive_vars_dtm()` in `ADSL`, `derive_param_computed()` in `ADVS`
-   Stuck? Ask your AI assistant before hand-coding a fix
:::
...
::: {.column width="50%"}
<div style="text-align: center;"> <img src="./images/adsl.png" alt="adsl" style="width: 100%; max-width: 550px;"/> </div>

* `ADSL` - one record per subject (306 obs, 48 vars). Focus: **adding variables**.
* Built from `{pharmaversesdtm}` (`dm`, `suppdm`, `ex`, `ae`, `vs`, etc.) using the `ADSL` section of our spec.
* Final version lives in [pharmaverseadam](https://pharmaverse.github.io/pharmaverseadam/), written out as an xpt file for regulatory send-off.
:::

::: {.column width="50%"}
<div style="text-align: center;"> <img src="./images/advs.png" alt="advs" style="width: 100%; max-width: 550px;"/> </div>

* `ADVS` - a Basic Data Structure (BDS) with **multiple records** per subject (~62K records, 68 vars).
* Built from `{pharmaversesdtm}` `vs` + some `ADSL` variables, using the `ADVS` section of our spec.
* Final version lives in [pharmaverseadam](https://pharmaverse.github.io/pharmaverseadam/); many functions repeat from the `ADSL` script.
:::
...
```{r, eval=FALSE}
file.edit("scripts/adsl.R")
file.edit("scripts/advs.R")
```

* Each opens in a new tab in your editor's Source pane.

## How We'll Dig Into the Scripts

::: incremental
-   Every slide from here on references specific line numbers in `adsl.R`/`advs.R` (footnoted like `^[Lines: 20-25]`) - follow along in your open script.
...
## Key Functions Cheat Sheet: `{admiral}` {.smaller}
...
reactable(
  ...
  columns = list(
    Package  = colDef(html = TRUE, minWidth = 110),
    Function = colDef(html = TRUE, minWidth = 220),
...
::: {.column width="75%"}
::: incremental
-   `admiral` derives variables & records driven by our Specs, and plays nice with tidyverse and base R
-   `_vars_` functions add **variables**; `_param_` functions add **records**
-   Click a function name to jump to its docs - or just ask your AI assistant
:::
:::

::: {.column width="25%"}
<div style="text-align: right;">
`r glue("<img src={admiral_img} height='100' style='display:inline-block;'/>")`
</div>
:::

::::

## Key Functions Cheat Sheet: `{metacore}`, `{metatools}`, `{xportr}` {.smaller}
...
::: {.column width="75%"}
::: incremental
-   `metacore`/`metatools` apply spec info to the ADaMs
-   `xportr` prepares the xpt file for regulatory agencies
-   Click a function name to jump to its docs - or just ask your AI assistant
:::
:::
...
::: {.column width="25%"}
<div style="text-align: right;">
`r glue("<img src={metacore_img} height='70' style='display:block; margin-left:auto;'/>")`
`r glue("<img src={metatools_img} height='70' style='display:block; margin-left:auto; margin-top:0.3em;'/>")`
`r glue("<img src={xportr_img} height='70' style='display:block; margin-left:auto; margin-top:0.3em;'/>")`
</div>
:::
...
* `verbose` argument should be set to `"silent"` while developing your spec and ADaMs (replaces the now-deprecated `quiet` argument).
...
* Goal: practice describing what you want in plain language and let the AI assistant map that to the right `admiral` function and arguments - then compare its choices to what you'd have written by hand.
...
* Goal: see how `admiral` can keep an imputed value from "wandering off" past dates you already know are true - useful when a partial date still needs to make logical sense next to other dates in the study.
...
* Goal: practice handing the AI a join scenario in plain language and seeing which arguments it reaches for to control ordering and filtering.
...
assumption were backwards.
```
:::

* Goal: practice a case where getting a single argument's direction wrong would silently produce the wrong numbers - a good habit to build before you're doing this on real data.
...
* Goal: practice a "one value applies to many records" scenario, which comes up any time a computed parameter mixes visit-level and subject-level inputs.
...
* Goal: see how a higher order function like `restrict_derivation()` lets you scope a derivation to a subset of records without losing the rest of the dataset.
...
* Goal: practice letting the AI reason about a setting you're genuinely unsure of, rather than only using it for settings you already know the answer to.
...
* Goal: build intuition for a one-line helper function by seeing it work (and fail gracefully) on data you understand.
...
* Goal: practice describing a rounding/truncation preference in plain terms and letting the AI translate it into the right combination of arguments.
...
* Goal: practice catching a silent data-quality problem (an unmapped code) instead of discovering it downstream.
...
* Goal: notice that `filter_add` scopes which rows are averaged *before* the calculation runs, rather than needing to handle missing values inside the formula itself.
...
* Goal: see how a strict `check_type` turns a silent data problem (duplicate ASEQ values) into something you're forced to notice and fix.
...
* Goal: practice the "last mile" of an ADaM - going from a labeled R data frame to a submission-ready xpt file - without having to memorize every xportr argument up front.
