...
::: incremental
-   🕙 10 mins: The possibilities of `tfrmt`

-   🕥 15 mins: Step-by-step formatting a display

-   🕚 10 mins: cards-to-tfrmt helpers

-   🕦 10 mins: Outputs using `docorator`
...
-   Takes a `gt` table (like our `tfrmt` output!), a `gt_group`, a `ggplot2` figure, or a PNG path

-   *Decorate*: wraps the display with document-level headers, footers, and page numbers

-   *Output*: renders the decorated display to PDF and/or RTF
...
2. Run the setup code, then copy/paste each prompt below into your AI assistant, one at a time.
...
## Prompt B: cards to tfrmt

::: {.small}
Paste this into your AI assistant:
:::

> Convert the `cards` object `ard_ae` into a tidy data frame ready for {tfrmt}. Keep only the required columns for the table. Ensure the names align to the spec above.

## Prompt C: Final AE table

::: {.small}
Paste this into your AI assistant:
:::

> Print a final AE table with real values by supplying the tfrmt-ready data. Add a title, subtitle, and footnote to the table.

## Prompt D: Output with docorator
