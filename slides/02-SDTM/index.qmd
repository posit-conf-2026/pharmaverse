...
Open **exercises/02-SDTM.R** — finish the **CM** domain. The walkthrough steps
are coded; one extra step (**CMINDC**) shows how to **prompt the AI** to fill a
mapping. Then complete the **7 exercises** — each has the aCRF annotation and a
suggested prompt.

- [✍️]{.pv-ico} **Code it yourself** — replace each `??` using the annotation text.
- [🤖]{.pv-ico} **Use the AI agent + skill** — paste the suggested prompt, then review and run.

```{r, eval = FALSE}
# === WALKTHROUGH (with AI): CMINDC — how I prompt the agent ============
