...
Each collected field maps with the algorithm that fits **how it was captured**:

::: {.pv-flow .pv-flow-accent}
[[✍️]{.pv-ico}assign_no_ct — free text]{.pv-step}
[[🏷️]{.pv-ico}assign_ct — coded dropdown]{.pv-step}
[[⚙️]{.pv-ico}hardcode_ct + condition_add]{.pv-step}
[[📅]{.pv-ico}assign_datetime — dates]{.pv-step}
:::

::: {.pv-ai}
[🔎]{.pv-ico} The aCRF annotation tells you which one: free text → `assign_no_ct`, coded list → `assign_ct`, conditional constant → `hardcode_ct`, a date → `assign_datetime`.
:::
...
**CMENRTPT** — set to `"Ongoing"` **only when** "Is medication ongoing?" is `Yes`.

![](images/cm-acrf-ongoing.png){.pv-acrf-img width="80%"}
...
::: {.pv-ai}
[⚙️]{.pv-ico} `condition_add()` filters the raw rows first; `hardcode_ct` then writes a **constant** CT value only for those rows.
:::
...
We just used an **agent + skill** to finish the CM domain. What are they?
